...
# There is a problem in this data....  Loggers did very strange stuff
...
Id_Site <- c("RAN-3")
Archipelago <- c("Tuamotu")
Island <- c("Rangiroa")
Site <- c("3")

# Introduce file names with the directory of the folder between " " where there are the CSV files
file.names = c("Data_Codes/Light/RAN3/RAN3-20-light.csv","Data_Codes/Light/RAN3/RAN3-40-light.csv","Data_Codes/Light/RAN3/RAN3-60-light.csv","Data_Codes/Light/RAN3/RAN3-90-light.csv","Data_Codes/Light/RAN3/RAN3-120-light.csv")
prof = c(20,40,60,90,120)
df_all=data.frame()

# Introduce limits of time, to get to the depth and to take it out. 
# Date_Time initial >> "2018-11-05 13:10:00"
# Date_Time final >>  "2018-11-12 11:00:00"
...
for(x in 1:length(file.names)){
  ...
  df <- subset(df, Date_Time > "2018-11-05 13:10:00" & Date_Time < "2018-11-12 11:00:00")
  df$Depth <- prof[x]
...
df_all_day <- subset(df_all, Time < "16:00:00" & Time > "07:00:00")
...
df_all$Id_Site <- "RAN3"
df_all$Archipelago <- "Tuamotu"
df_all$Island <- "Rangiroa"
df_all$Site <- "3"
...
write.csv(df_all, "Data_Codes/Light/RAN3/RAN3_Light_data.csv")
...
Index_light$Id_Site <- "RAN3"
Index_light$Archipelago <- "Tuamotu"
Index_light$Island <- "Rangiroa"
Index_light$Site <- "3"
...
write.csv (Index_light_Loss, paste("Data_Codes/Light/RAN3/",Id_Site, "_","Index_Light_loss.csv", sep = "")) # Database used for figure in Physiology manuscript
